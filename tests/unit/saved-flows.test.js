...
describe('SavedFlows', () => {
  ...
  it('listFor() returns [] for an unknown user', () => {
    expect(window.SavedFlows.listFor('nobody@example.com')).toEqual([]);
  });

  it('listFor() returns [] when email is empty', () => {
    expect(window.SavedFlows.listFor('')).toEqual([]);
  });

  it('save() prepends new flows so most recent is first', () => {
    const email = 'a@b.com';
  ...
  it('remove() deletes the matching flow', () => {
    const email = 'a@b.com';
